...
import PageProject from '../../app/project/page-project';
          ...
          <div className="grid grid-flow-col justify-end gap-2 md:gap-4">
            {filterOptions.map((item) => (
              <Button
                ...
                className={`rounded-full p-2 px-8 text-white hover:bg-black !hover:text-white transition-colors ${
                  item.value === filter
                    ? 'bg-black'
            ...
            ? projects
                ...
                .map(
                  (
                    ...
                    return <PageProject item={item} key={item.slug} />;
                  }
                )
            : Array(6)
                .fill('')
                .map((_: any, index: any) => (
                  <div
                    key={index}
                    className="rounded-[20px] h-[450px] w-full bg-slate-200 flex hover:shadow-md animate-pulse relative border"
                  ></div>
                ))}
        </div>
      </div>
